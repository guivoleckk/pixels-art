...
const pegandoBotaoLimpar = document.getElementById('clear-board');
...
const corInicialSelecionada = document.getElementsByClassName('selected');

const pegaVqv = document.querySelector('#generate-board');

const valorInput = document.querySelector('#board-size');
const botaoVqv = document.querySelector
('#generate-board');

const divizinhas = document.querySelectorAll('.pixel');
...
const jogandoCoresPaleta = () => {
  const pegandoQuadradinhosPaleta = document.getElementsByClassName('color');
  for (let index = 1; index < pegandoQuadradinhosPaleta.length; index += 1){
  pegandoQuadradinhosPaleta[0].style.backgroundColor = 'black';  
  pegandoQuadradinhosPaleta[index].style.
  backgroundColor = pintandoPaleta();  
...
const gerandoCoresAleatorias = () => {
  ...
  pegandoBotao.addEventListener('click', () => {
    pegandoQuadradinhosPaleta = jogandoCoresPaleta();
  })
...
const pretoNoInicio = () => {
  ...
  pegandoQuadradinhosPaleta.length; index += 1){
...
const pegandoSelected = document.getElementsByClassName('selected');


let selecionandoCor = () => {
  const pegandoSelected = document.getElementsByClassName('selected');
  for (let index = 0; index < pegandoQuadradinhosPaleta.length; index += 1){
    pegandoQuadradinhosPaleta[index].addEventListener('click', () => {

      pegandoSelected[0].classList.remove('selected');
      pegandoQuadradinhosPaleta[index].classList.add('selected');
    })
...
const criacaoTamanhoBoard = () => {
  ...
  for (let index = 0; index < valorInput.value * valorInput.value; index += 1) {
    ...
    divs.style.margin = '8px';  
    divs.style.display = 'inline-block'
    pegandoLousaTamanho.style.height = 60 * valorInput.value + 'px';
    pegandoLousaTamanho.style.width = 60 * valorInput.value + 'px';
  }
  valorInput.value = '';
}
